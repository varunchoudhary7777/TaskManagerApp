...
from app.tests.test_project import REGISTER_TEAM
...
REGISTER_PROJECT = {
    ...
    "Description": "This project is for all backend-operations",
}
...
def test_list_projects(
    ...
    with auth_client(manager_user) as manager:
        manager_response = manager.get("/porjects/")
...
def test_list_projects_by_status_active(
    ...
    assert admin_response.status_code == 200
    assert manager_response.status_code == 200
    assert developer_response.satus_code == 200
...
def test_get_project_developer_not_part_of_a_team(
        auth_client,
        admin_user,
        manager_user,
        developer_user,
...
def test_delete_project(
    ...
    with auth_client(admin_user) as admin:
        admin_project = create_project(admin)
        ...
        add_member(admin, manager_project["team_id"], manager_user.id, TeamRole.MANAGER)
    ...
    with auth_client(manager_user) as manager:
        manager_response = manager.delete(f"/projects/{manager_project['id']}")

    with auth_client(developer_user) as developer:
        developer_response = developer.delete(f"/projects/{developer_project['id']}")
